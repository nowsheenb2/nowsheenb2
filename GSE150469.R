...
write.csv(sig.limma,file ="DEMRNA_RNASeq.csv")
...
write_csv(downregulated_mrna, "downregulated_RNASeqMRNA.csv")
...
write_csv(upregulated_mrna, "upregulated_RNASeqMRNA.csv")
...
diseased_vs_healthy %>%
  ...
  scale_x_continuous(breaks = c(seq(-15, 15, 2)),       
                     limits = c(-15, 15))+
  labs(title = "Volcano plot of differentially expressed mRNA in GSE150469",
       x = "log2(Fold Change)",
       y = "-log10(Adj.P.Value)",
       colour = "Expression \nchange") +
